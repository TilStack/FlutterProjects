  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: Image.asset("assets/png/till.png", height: 10, width: 10,),
      ),
      body: SpinCircleBottomBarHolder(
        bottomNavigationBar: SCBottomBarDetails(
          ...
          titleStyle: GoogleFonts.poppins(color: Colors.black45,fontSize: 12),
          activeTitleStyle: GoogleFonts.poppins(color:SECOND_COLOR,fontSize:12, fontWeight: FontWeight.bold),
          actionButtonDetails: SCActionButtonDetails(color: SECOND_COLOR,icon: Icon(Icons.expand_less),elevation: 0),
          items: [
            ...
            SCBottomBarItem(icon: Icons.details, onPressed: (){},title: 'Details'),
          ],
          circleItems: [
                  ...
                  child: Row(
                    ...
                    children: [
                      Container(
                        ...
                        child: Row(
                          children: [
                            Container(margin: const EdgeInsets.only(left: 10),child: Icon(Icons.search, color: Colors.black,)),
                            Text('Search', style: GoogleFonts.poppins(color: Colors.grey),)
                          ],
                        ),
                      ),
                      Container(margin: const EdgeInsets.only(left: 5),decoration: BoxDecoration(color: FIRST_COLOR,borderRadius: BorderRadius.circular(10)),child: IconButton(onPressed: (){}, icon: Icon(Icons.list, color: SECOND_COLOR,))),
                    ],
                  ),
                ),
              ),
              const CardContent(title: 'My First Title',subtitle:'The name given to a book, movie, song, or any other creative work. ',image: "assets/png/till.png",),
              CategoryCard(title: 'Category', all: 'See All'),
              SizedBox(
                height: 100,
                width: size.width/1.2,
                child: ListView.builder(itemCount: _items.length,scrollDirection: Axis.horizontal,
                    itemBuilder: (context, index){
                      final item = _items[index];
                  return Item(title: item['title'], iconData: item['icon'],);
                }),
              ),
              SizedBox(height: 10,),
